	...
	if (renderMethod == BB_METHOD_ACCELERATED){
		...
		if (++selfInstances == 1){
			...
			if (Root::getSingleton().getRenderSystem()->getName() == "Direct3D9 Rendering Subsystem")
				...
				shaderLanguage = "glsl";
			else if(Root::getSingleton().getRenderSystem()->getName() == "OpenGL ES 2.x Rendering Subsystem")
				shaderLanguage = "glsles";
			else
				shaderLanguage = "cg";

			Ogre::LogManager::getSingleton().logMessage("PagedGeometry: StaticBillboardSet: shaderLanguage: " + shaderLanguage);// + Ogre::StringConverter::toString());
			...
			if (vertexShader.isNull()){
				...
				else if (!shaderLanguage.compare("glsles"))
				{
					// GLSLES
					vertexProg =
					"#version 100\n"
					...
					"varying vec2 oUV0; \n"
					"varying float oFadeAlpha;\n"
					"void main() { \n"
					//Face the camera
					"	vec4 vCenter = vec4( position.x, position.y, position.z, 1.0 ); \n"
			...
			if (vertexShader2.isNull()){
				...
				else if(!shaderLanguage.compare("glsles"))
				{
					...
					"	float dist = distance(camPos.xz, position.xz); \n"
					"	oFadeAlpha = (invisibleDist - dist) / fadeGap; \n"
				...
				// Generate fragment shader for GLES2
				if (shaderLanguage == "glsles")
				{
					...
					"varying float oFadeAlpha;\n"
					"void main() { \n"
					"	vec4 texCol = texture2D(tex, oUV0); \n"
					//"	vec4 texCol = vec4(oUV0.xy, 0.0, 1.0); \n"
					"	gl_FragColor = texCol; \n"
					"	gl_FragColor.a *= oFadeAlpha;\n"
					"}";

					HighLevelGpuProgramPtr fragmentShader = HighLevelGpuProgramManager::getSingleton().createProgram("Sprite_fp", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, shaderLanguage, GPT_FRAGMENT_PROGRAM);
					fragmentShader->setSource(fragmentProg);
					fragmentShader->load();
		...
		fallbackSet = sceneMgr->createBillboardSet(100);
		fallbackSet->setName(getUniqueID("SBS"));
...
MaterialPtr StaticBillboardSet::getFadeMaterial(Real visibleDist, Real invisibleDist)
{
	...
	if (it != fadedMaterialMap.end()){
		...
		bool isglsl = false, isglsles = false;
		...
		for (unsigned short t = 0; t < fadeMaterial->getNumTechniques(); ++t){
			...
			for (unsigned short p = 0; p < tech->getNumPasses(); ++p){
				...
				if (isglsles) {
					pass->setFragmentProgram("Sprite_fp");
					pass->getFragmentProgramParameters()->setNamedConstant("tex", 0);
				}
			}
		}
...
void StaticBillboardSet::updateAll(const Vector3 &cameraDirection)
{
	if (selfInstances > 0){  //selfInstances will only be greater than 0 if one or more StaticBillboardSet's are using BB_METHOD_ACCELERATED
		...
		while (i1 != i2){
			...
			if (isglsles && !p->hasFragmentProgram()){
				p->setFragmentProgram("Sprite_fp");
				//p->getFragmentProgramParameters()->setNamedAutoConstant("worldViewProj", GpuProgramParameters::ACT_WORLDVIEWPROJ_MATRIX);
			}
