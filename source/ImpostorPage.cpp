...
void ImpostorPage::addEntity(Entity *ent, const Vector3 &position, const Quaternion &rotation, const Vector3 &scale, const Ogre::ColourValue &color)
{
	...
	// hack 2.0
	center.y += position.y + ent->getMesh()->getBounds().getCenter().y * scale.y;
	++aveCount;
}
...
{
	//LogManager::getSingleton().logMessage("ImpostorPage::update");
...
void ImpostorBatch::setAngle(float pitchDeg, float yawDeg)
{
	...
	if (newPitchIndex != pitchIndex || newYawIndex != yawIndex){
		...
		//Ogre::LogManager::getSingleton().logMessage("PagedGeometry: ImpostorPage: setMaterial: " + tex->material[pitchIndex][yawIndex]->getName());// + Ogre::StringConverter::toString());
...
: loader(0)
{
	...
	// hack 2.0
	boundingBox = entity->getMesh()->getBounds();
	...
	for (int i = 0; i < IMPOSTOR_PITCH_ANGLES; ++i){
		...
		//Ogre::LogManager::getSingleton().logMessage("PagedGeometry: ImpostorTexture::ImpostorTexture: created material: " + m->getName());
...
void ImpostorTexture::renderTextures(bool force)
{
	...
	String oldMat = entity->getSubEntity(0)->getMaterialName();
	String mymatname = entity->getSubEntity(0)->getMaterialName() + "_impostor";
	MaterialPtr mymat = entity->getSubEntity(0)->getMaterial()->clone(mymatname);
	...
	if (!shaderLanguage.compare("glsl")) {
		mymat->getTechnique(0)->getPass(0)->setVertexProgram("SimpleVp_GLSL");
		mymat->getTechnique(0)->getPass(0)->setFragmentProgram("SimpleFp_GLSL");
	} else if (!shaderLanguage.compare("glsles")) {
		mymat->getTechnique(0)->getPass(0)->setVertexProgram("SimpleVp_GLSLES");
		mymat->getTechnique(0)->getPass(0)->setFragmentProgram("SimpleFp_GLSLES");
	}
	entity->setMaterialName(mymatname);
