...
unsigned long TestBatchPage::refCount = 0;
unsigned long TestBatchPage::GUID = 0;


void TestBatchPage::init(PagedGeometry *_geom, const Any &data)
{
	geom = _geom;
	...
	batch = new TestBatchedGeometry(sceneMgr, geom->getSceneNode());
...
void TestBatchPage::addEntity(Entity *ent, const Vector3 &position, const Quaternion &rotation, const Vector3 &scale, const Ogre::ColourValue &color)
{
	...
	if ( mLODLevel > 0 && numManLod < mLODLevel )
		Ogre::LogManager::getSingleton().logMessage( "TestBatchPage::addEntity: " + ent->getName() + " entity has less than " + Ogre::StringConverter::toString(mLODLevel) + " manual lod level(s). Performance warning.");
#endif
...
void TestBatchPage::build()
{
	batch->build();

	TestBatchedGeometry::SubBatchIterator it = batch->getSubBatchIterator();
	while (it.hasMoreElements()){
		TestBatchedGeometry::SubBatch *subBatch = it.getNext();
...
void TestBatchPage::removeEntities()
{
	batch->clear();
...
void TestBatchPage::setVisible(bool visible)
{
	batch->setVisible(visible);
}

void TestBatchPage::setFade(bool enabled, Real visibleDist, Real invisibleDist)
{
	if (!shadersSupported)
		return;
...
void TestBatchPage::_updateShaders()
{
	if (!shadersSupported)
		return;
...
void TestBatchPage::update()
{
	//Ogre::LogManager::getSingleton().logMessage("TestBatchPage::update");
//	batch->update();
}
