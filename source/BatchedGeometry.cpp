...
void BatchedGeometry::build()
{
	...
	if (subBatchMap.size() != 0) {
		...
		if (built) sceneNode->attachObject(this);
...
void BatchedGeometry::SubBatch::build()
{
	...
	QueuedMesh myqm;
	myqm = meshQueue.back();
	while (n_meshQueue < 10 && meshQueue.size()) {
	...
	LogManager::getSingleton().logMessage("PAGED_GEOMETRY: BatchedGeometry: meshes added: " + Ogre::StringConverter::toString(n_meshQueue));
...
{
	LogManager::getSingleton().logMessage("BatchedGeometry::SubBatch::clear");
