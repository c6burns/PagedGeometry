...
TestBatchedGeometry::TestBatchedGeometry(SceneManager *mgr, SceneNode *rootSceneNode)
 :	withinFarDistance(0),
...
TestBatchedGeometry::~TestBatchedGeometry()
{
	clear();
}

void TestBatchedGeometry::addEntity(Entity *ent, const Vector3 &position, const Quaternion &orientation, const Vector3 &scale, const Ogre::ColourValue &color)
{
	...
	for (uint32 i = 0; i < ent->getNumSubEntities(); ++i){
		...
		if (subMesh->vertexData == NULL)
			OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR, "SubMesh vertex data not found!", "TestBatchedGeometry::addEntity()");
...
uint32 TestBatchedGeometry::CountUsedVertices(IndexData *id, std::map<uint32, uint32> &ibmap)
{
	uint32 i, count;
...
void TestBatchedGeometry::extractVertexDataFromShared(MeshPtr mesh)
{
...
TestBatchedGeometry::SubBatchIterator TestBatchedGeometry::getSubBatchIterator() const
{
	return TestBatchedGeometry::SubBatchIterator((SubBatchMap&)subBatchMap);
}

String TestBatchedGeometry::getFormatString(SubEntity *ent)
{
...
void TestBatchedGeometry::update()
{
	if (subBatchMap.size() != 0) {
		for (SubBatchMap::iterator i = subBatchMap.begin(); i != subBatchMap.end(); ++i){
			i->second->build_continue();
		}
	}
}

void TestBatchedGeometry::build()
{
	///Make sure the batch hasn't already been built
	if (built)
		OGRE_EXCEPT(Exception::ERR_DUPLICATE_ITEM, "Invalid call to build() - geometry is already batched (call clear() first)", "TestBatchedGeometry::GeomBatch::build()");
...
void TestBatchedGeometry::clear()
{
...
void TestBatchedGeometry::_updateRenderQueue(RenderQueue *queue)
{
	//If visible...
	if (isVisible()){
...
bool TestBatchedGeometry::isVisible()
{
	return mVisible && withinFarDistance;
}

void TestBatchedGeometry::_notifyCurrentCamera(Camera *cam)
{
...
Ogre::Vector3 TestBatchedGeometry::_convertToLocal(const Vector3 &globalVec) const
{
...
TestBatchedGeometry::SubBatch::SubBatch(TestBatchedGeometry *parent, SubEntity *ent)
	: indexOffset(0)
	, vertexOffset(0)
	, m_vertBinding(NULL)
	, m_vertDecl(NULL)
	, m_indexBytes(0)
{
	...
	if (origMat) {
		...
		if (result.first.isNull()) {
			OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "BatchedGeometry failed to create a material for entity with invalid material.", "TestBatchedGeometry::SubBatch::SubBatch(BatchedGeometry *parent, SubEntity *ent)");
		}
...
TestBatchedGeometry::SubBatch::~SubBatch()
{
	clear();
...
Material *TestBatchedGeometry::SubBatch::getMaterialClone(Material &mat)
{
...
void TestBatchedGeometry::SubBatch::addSubEntity(SubEntity *ent, const Vector3 &position, const Quaternion &orientation, const Vector3 &scale, const Ogre::ColourValue &color, void* userData)
{
	assert(!built);
	...
	if (newMesh.color != ColourValue::White) {
		...
		switch (format){
				...
				case VET_COLOUR_ABGR:
					break;
				default:
					OGRE_EXCEPT(0, "Unknown RenderSystem color format", "TestBatchedGeometry::SubBatch::addSubMesh()");
					break;
		}
	}
...
void TestBatchedGeometry::SubBatch::build_continue()
{
	int ms = meshQueue.size();
	if (!ms) {
		// nothing to build
		return;
	}
	LogManager::getSingleton().logMessage("TestBatchedGeometry::SubBatch::build_continue: " + Ogre::StringConverter::toString(ms) + " queued meshes / " + StringConverter::toString(m_indexBytes) + " / " + StringConverter::toString(m_vertexBytes[0]) + " / " + StringConverter::toString(m_vertexBytes[1]));

	//Lock the index buffer
	if (m_destIndexType == HardwareIndexBuffer::IT_32BIT)
		m_indexBuffer32 = static_cast<uint32*>(indexData->indexBuffer->lock(m_indexBytes, (indexData->indexBuffer->getSizeInBytes() - m_indexBytes), HardwareBuffer::HBL_WRITE_ONLY));
	else
		m_indexBuffer16 = static_cast<uint16*>(indexData->indexBuffer->lock(m_indexBytes, (indexData->indexBuffer->getSizeInBytes() - m_indexBytes), HardwareBuffer::HBL_WRITE_ONLY));

	//Allocate & lock the vertex buffers
	m_vertBinding = vertexData->vertexBufferBinding;
	m_vertDecl = vertexData->vertexDeclaration;

	m_vertexBuffers.clear();
	m_vertexBufferElements.clear();
	for (Ogre::ushort i = 0; i < m_vertBinding->getBufferCount(); ++i)
	{
		//HardwareVertexBufferSharedPtr buffer = HardwareBufferManager::getSingleton().createVertexBuffer(m_vertDecl->getVertexSize(i), vertexData->vertexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
		HardwareVertexBufferSharedPtr buffer = m_vertBinding->getBuffer(i);
		
		m_vertexBuffers.push_back(static_cast<uchar*>(buffer->lock(m_vertexBytes[i], (buffer->getSizeInBytes() - m_vertexBytes[i]), HardwareBuffer::HBL_WRITE_ONLY)));
		m_vertexBufferElements.push_back(m_vertDecl->findElementsBySource(i));
	}
	...
	if (requireVertexColors) {
		if (!vertexData->vertexDeclaration->findElementBySemantic(VES_DIFFUSE)) {
			Ogre::ushort i = (Ogre::ushort)m_vertBinding->getBufferCount();

			m_vertDecl->addElement(i, 0, VET_COLOUR, VES_DIFFUSE);

			HardwareVertexBufferSharedPtr buffer = HardwareBufferManager::getSingleton()
				.createVertexBuffer(m_vertDecl->getVertexSize(i), vertexData->vertexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
			m_vertBinding->setBinding(i, buffer);
	...
	int citer = 0;
	while (citer < MESHES_PER_FRAME && meshQueue.size()) {
		citer++;
		build_increment();
	}
	...
	indexData->indexBuffer->unlock();
	for (Ogre::ushort i = 0; i < m_vertBinding->getBufferCount(); ++i)
		m_vertBinding->getBuffer(i)->unlock();

	built = true;
}

void TestBatchedGeometry::SubBatch::build_increment()
{
	if (!meshQueue.size()) {
		LogManager::getSingleton().logMessage("TestBatchedGeometry::SubBatch::build_increment: error, nothing to build!!!");
		return;
	}

	LogManager::getSingleton().logMessage("TestBatchedGeometry::SubBatch::build_increment: " + StringConverter::toString(m_indexBytes) + " / " + StringConverter::toString(m_vertexBytes[0]) + " / " + StringConverter::toString(m_vertexBytes[1]));
	const QueuedMesh queuedMesh = meshQueue.back();
	meshQueue.pop_back();
	...
	for (Ogre::ushort i = 0; i < destBinds->getBufferCount(); ++i)
	{
		...
		if (i < sourceBinds->getBufferCount()){
			...
			uchar *destBase = m_vertexBuffers[i];
			...
			for (size_t v = 0; v < sourceVertexData->vertexCount; ++v)
			{
				...
				VertexDeclaration::VertexElementList &elems = m_vertexBufferElements[i];
				...
				for (ei = elems.begin(); ei != elems.end(); ++ei)
				{
					...
					case VES_POSITION:
						...
						tmp -= m_batchCenter;		//Adjust for batch center
					...
					nbytes += VertexElement::getTypeSize(elem.getType());
				}
			...
			m_vertexBuffers[i] = destBase;
			sourceBuffer->unlock();
		} else {
			assert(requireVertexColors);

			//Get the locked output buffer
			uint32 *startPtr = (uint32*)m_vertexBuffers[m_vertBinding->getBufferCount()-1];
			...
			m_vertexBuffers[m_vertBinding->getBufferCount()-1] += (sizeof(uint32) * sourceVertexData->vertexCount);
			nbytes += sizeof(uint32) * sourceVertexData->vertexCount;
		}

		m_vertexBytes[i] += nbytes;
	}


	//Copy mesh index data into the index buffer
	if (m_srcIndexType == HardwareIndexBuffer::IT_32BIT) {
		//Lock the input buffer
		...
		while (source != sourceEnd) {
			*m_indexBuffer32++ = static_cast<uint32>(*source++ + indexOffset);
			m_indexBytes += sizeof(uint32);
		}
		...
		if (m_destIndexType == HardwareIndexBuffer::IT_32BIT){
			...
			while (source != sourceEnd) {
				uint32 indx = *source++;
				*m_indexBuffer32++ = (indx + indexOffset);
				m_indexBytes += sizeof(uint32);
			}
			...
			//And copy it to the output buffer
			while (source != sourceEnd) {
				*m_indexBuffer16++ = static_cast<uint16>(*source++ + indexOffset);
				m_indexBytes += sizeof(uint16);
			}
...
void TestBatchedGeometry::SubBatch::build()
{
	assert(!built);

	int ms = meshQueue.size();
	LogManager::getSingleton().logMessage("TestBatchedGeometry::SubBatch::build: initial build started on " + Ogre::StringConverter::toString(ms) + " queued meshes");
	//if (!ms) {
		// nothing to build
	//	return;
	//}
	...
	//Misc. setup
	m_batchCenter = parent->center;

	m_srcIndexType = meshType->indexData->indexBuffer->getType();
	if (vertexData->vertexCount > 0xFFFF || m_srcIndexType == HardwareIndexBuffer::IT_32BIT)
		m_destIndexType = HardwareIndexBuffer::IT_32BIT;
	else
		m_destIndexType = HardwareIndexBuffer::IT_16BIT;

	//Allocate the index buffer
	indexData->indexBuffer = HardwareBufferManager::getSingleton().createIndexBuffer(m_destIndexType, indexData->indexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);

	//Lock the index buffer
	if (m_destIndexType == HardwareIndexBuffer::IT_32BIT)
		m_indexBuffer32 = static_cast<uint32*>(indexData->indexBuffer->lock(HardwareBuffer::HBL_WRITE_ONLY));
	else
		m_indexBuffer16 = static_cast<uint16*>(indexData->indexBuffer->lock(HardwareBuffer::HBL_WRITE_ONLY));
	...
	for (Ogre::ushort i = 0; i < m_vertBinding->getBufferCount(); ++i)
	{
		...
		m_vertexBuffers.push_back(static_cast<uchar*>(buffer->lock(HardwareBuffer::HBL_WRITE_ONLY)));
		m_vertexBufferElements.push_back(m_vertDecl->findElementsBySource(i));
	}
...
void TestBatchedGeometry::SubBatch::clear()
{
...
void TestBatchedGeometry::SubBatch::addSelfToRenderQueue(RenderQueue *queue, uint8 group)
{
	if (built){
...
void TestBatchedGeometry::SubBatch::getRenderOperation(RenderOperation& op)
{
...
Real TestBatchedGeometry::SubBatch::getSquaredViewDepth(const Camera* cam) const
{
...
const Ogre::LightList& TestBatchedGeometry::SubBatch::getLights(void) const
{
