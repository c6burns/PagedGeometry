...
void BatchPage::addEntity(Entity *ent, const Vector3 &position, const Quaternion &rotation, const Vector3 &scale, const Ogre::ColourValue &color)
{
	...
	else
	{
		...
		//Ogre::LogManager::getSingleton().logMessage( "BatchPage::addEntity: " + ent->getName() + ": lod: " + Ogre::StringConverter::toString(bestLod));
		batch->addEntity(lod, position, rotation, scale, color);
	}
}

void BatchPage::update()
{
	if (!batch->built) {
		batch->build();
	}
...
void BatchPage::_updateShaders()
{
	...
	while (it.hasMoreElements()){
		...
		if (HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName).isNull())
		{
			...
			else if(!shaderLanguage.compare("glsl"))
			{
				vertexProgSource =
					"#version 120\n"
			...
			else if(!shaderLanguage.compare("glsles"))
			{
				...
				//Ogre::LogManager::getSingleton().logMessage("GLES2 vertexProgSource: \n"+vertexProgSource);
			}
		...
		if (HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName+"FP").isNull())
		{
			Pass *pass = mat->getTechnique(0)->getPass(0);
			...
			if (!shaderLanguage.compare("glsl"))
			{
				fragmentProgSource =
					...
					"void main()\n"
					"{\n"
					"	gl_FragColor = texture2D(tex, oUV0);//vec4(1.0, 0.0, 0.0, 1.0);\n"
			...
			else if (!shaderLanguage.compare("glsles"))
			{
				fragmentProgSource =
					...
					"	gl_FragColor.a *= oFadeAlpha;\n"
					"}\n";
				//Ogre::LogManager::getSingleton().logMessage("GLES2 fragmentProgSource: \n"+fragmentProgSource);
			}

			HighLevelGpuProgramPtr fragmentShader = HighLevelGpuProgramManager::getSingleton().createProgram(
				vertexProgName+"FP",
				ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
				shaderLanguage, GPT_FRAGMENT_PROGRAM);

			fragmentShader->setSource(fragmentProgSource);
			...
			for (unsigned short t = 0; t < generatedMaterial->getNumTechniques(); ++t){
				...
				for (unsigned short p = 0; p < tech->getNumPasses(); ++p){
					...
					if (pass->getFragmentProgramName() == "")
						pass->setFragmentProgram(vertexProgName+"FP");
